...
- Q5 多头净值、回撤和成本影响图：看最高组因子组合在简化调仓规则下是否还能成立。
...
- 计算 5 组分组收益。
- 计算最高组 - 最低组多空收益。
- 补充轻量研究型回测分析：Q5 等权、5 日调仓、单边成本、净值曲线、回撤和核心绩效指标。
- 输出前半段 / 后半段稳定性。
- 输出 PIT 风险简表。
- 画必要图表。
- Notebook 从头到尾可顺序运行。
...
- 不做完整多因子合成模型。
- 不做复杂组合优化。
- 不做正式交易回测系统，只做 Notebook 内的轻量研究型回测分析。
- 不做实盘成交模拟；交易成本只用固定单边费率做敏感性提示。
- 不做机器学习模型。
- 不做 dashboard。
